...
public class ImmutableServerRepository extends AbstractResourceRepository<ImmutableServer, ImmutableServerConfig> {

    private final static String RESOURCE_NAME = "App";

    @Override
    public ImmutableServer create(Construct scope, String namespace, Environment stage, ImmutableServerConfig conf) {
        return new ImmutableServer(scope, RESOURCE_NAME, conf);
    }
    ...
    public List<CfnOutput> export(Construct scope, ImmutableServer resource) {
        ...
        outputs.add(createOutput(scope, "AsgArn", "The AutoScalingGroup Arn", resource.getAsg().getAutoScalingGroupArn()));
